...
public class State implements Comparable<Object> {
    ...
    public void initializeChildren() { 
    	State[] ans = new State[board.getPossibleMoves(player).length];
    	int position = 0;
    	for (Move x: board.getPossibleMoves(player)) {
        	Board tmp = new Board(board, x);
        	ans[position] = new State(player, tmp, x);
        	position++;
        }
        setChildren(ans);
    }
